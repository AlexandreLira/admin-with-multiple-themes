...
const ThemeMenu = () => {
    ...
    useEffect(() => {
        ...
        setCurrentMode(themeClass !== undefined && themeClass.id)
        setCurrentColor(colorClass !== undefined && colorClass.id)
    }, [])
                    ...
                    <ul className="mode-list">
                        {
                            mode_settings.map((item, index) => (
                                <li key={index} onClick={() => setMode(item)}>
                                    <div className={`mode-list_color ${item.background} ${item.id === currentMode ? 'active' : ''}`}>
                    ...
                    <ul className="mode-list">
                        {
                            color_settings.map((item, index) => (
                                <li key={index} onClick={() => setColor(item)}>
                                    <div className={`mode-list_color ${item.background} ${item.id === currentColor ? 'active' : ''}`}>
                                        <i className="bx bx-check"></i>
                                    </div>
                                    <span>{item.name}</span>
                                </li>
